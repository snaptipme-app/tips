  ...
  return(<>
    {detail&&<div onClick={()=>{setDetail(null);setRejectMode(false);setRejectReason('');}} style={{position:'fixed',inset:0,zIndex:9998,background:'rgba(0,0,0,.82)',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:20,overflowY:'auto'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:CARD,border:'1px solid rgba(255,255,255,0.06)',borderRadius:24,padding:28,maxWidth:560,width:'100%',marginTop:20,marginBottom:20}}>
        ...
        {[['Method',detail.method,'text'],['Payout Method',detail.payout_method||'wise_manual','text'],['Payout Status',detail.payout_status||detail.status,'payoutBadge'],['Schedule',detail.payout_schedule||'manual','text'],['Status',null,'badge'],['Stripe Transfer',detail.stripe_transfer_id||'Not created','text'],['Requested',fmtDate(detail.created_at),'text'],['Processed',detail.processed_at?fmtDate(detail.processed_at):'Not processed','text']].map(([l,v,t])=><div key={l} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:'1px solid rgba(255,255,255,0.06)'}}><span style={{color:'rgba(255,255,255,.4)',fontSize:13}}>{l}</span>{t==='badge'?<StatusBadge status={detail.status}/>:t==='payoutBadge'?<StatusBadge status={v}/>:<span style={{color:'#fff',fontSize:13,fontWeight:600,textTransform:l==='Payout Method'||l==='Schedule'||l==='Payout Status'?'capitalize':'none'}}>{v}</span>}</div>)}
    ...
    <div style={{background:CARD,borderRadius:16,border:'1px solid rgba(255,255,255,0.06)',overflow:'auto'}}>
      ...
      <tbody>{filtered.map(w=><tr key={w.id}>
        ...
        <td style={{fontSize:12,color:'rgba(255,255,255,.6)',maxWidth:150}}><div>{w.method}</div><div style={{fontSize:11,color:ACCENT,textTransform:'capitalize'}}>{w.payout_method||'wise_manual'} / {w.payout_status||w.payout_schedule||'manual'}</div></td>
