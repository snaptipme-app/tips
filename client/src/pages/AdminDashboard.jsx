...
function UsersSection({showToast,onLogout}){
  ...
  const doAction=async(action,userId,msg)=>{setActionLoading(userId);try{if(action==='suspend')await api().patch(`/users/${userId}/suspend`);else if(action==='reactivate')await api().patch(`/users/${userId}/reactivate`);else if(action==='delete')await api().delete(`/users/${userId}`);else if(action==='reset'){const r=await api().post(`/users/${userId}/reset-password`);showToast(r.data.message);setActionLoading(null);fetchUsers();return}showToast(msg);fetchUsers()}catch(e){showToast(e.response?.data?.error||'Failed','error')}setActionLoading(null);setConfirm(null)};
